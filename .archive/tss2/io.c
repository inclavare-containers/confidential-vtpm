...
#ifdef RATS_TLS
// TODO: I change the TPM command socket into block socket. And can we use the nonblock socket?
ssize_t
read_all (
    ...
    size_t size)
{
    ...
    // LOG_DEBUG ("reading %zu bytes from fd %s to buffer at 0x%" PRIxPTR,
    //            size, handle, (uintptr_t)data);
// 
    while(recvd_total < size)
    {
        ret = rats_tls_receive(handle, data + recvd_total, &recvd);
        
        if (ret == -TLS_WRAPPER_ERR_RECEIVE && recvd == size){
    ...
    size_t size)
{
    ...
    int ret;
    uint8_t *buf_back = (uint8_t *) buf;

    while(written_total < size)
    {

        ret = rats_tls_transmit(handle, buf_back + written_total, &written);
        // printf("%s!\n", buf_back);
        if (ret != RATS_TLS_ERR_NONE) {
...
socket_recv_buf (
    ...
    size_t size)
{
    return read_all (handle, data, size);
}
    ...
    size_t size)
{
    ...
    // LOGBLOB_DEBUG (buf, size, "Writing %zu bytes to handle %s:", size, handle);
    ret = write_all (handle, buf, size);
    if (ret < (ssize_t) size) {
        // LOG_ERROR ("write to fd %s failed, errno %d: %s", handle, errno, strerror (errno));
        return TSS2_TCTI_RC_IO_ERROR;
    }
