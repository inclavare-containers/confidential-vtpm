...
/*
#############################################################################
General-purpose functions 1.1:
tcti_mssim_context_cast
tcti_mssim_down_cast
...
/*
#############################################################################
Rats-TLS-enhanced-purpose functions:
    ...
    UINT32 cmd)
{
    ...
    ret = write_all (tcti_mssim->platform_handle, buf, sizeof (cmd));
    ...
    ret = rats_tls_receive(tcti_mssim->platform_handle, buf, &len);
        if (ret != RATS_TLS_ERR_NONE) {
            RTLS_ERR("Failed to receive %#x\n", ret);
            rats_tls_cleanup(tcti_mssim->platform_handle);
            return TSS2_TCTI_RC_IO_ERROR;
        }
    ...
    rats_tls_handle handle)
{
    ...
    return socket_xmit_buf (handle, buf, sizeof (buf));
}
    ...
    UINT32 size)
{
    ...
    return socket_xmit_buf (tcti_mssim->tpm_handle, buf, sizeof (buf));
}
    ...
    const uint8_t *cmd_buf)
{
    ...
    rc = socket_xmit_buf (tcti_mssim->tpm_handle, cmd_buf, size);
    if (rc != TSS2_RC_SUCCESS) {
        return rc;
    }
    ...
    TSS2_TCTI_CONTEXT *tctiContext)
{
    ...
    send_sim_session_end (tcti_mssim->platform_handle);
    send_sim_session_end (tcti_mssim->tpm_handle);
    ...
    int32_t timeout)
{
    ...
    if (tcti_common->header.size == 0) {
        ...
        ret = socket_recv_buf (tcti_mssim->tpm_handle, size_buf, sizeof(UINT32));
    ...
    *response_size = tcti_common->header.size;
    ...
    ret = socket_recv_buf (tcti_mssim->tpm_handle,
                           (unsigned char *)response_buffer,
                           tcti_common->header.size);
...
/*
#############################################################################
Main function Tss2_Tcti_Mssim_Init
#############################################################################
*/
/*
    ...
    const char *conf)
{
    ...
    // tcti_mssim->tpm_handle = -1;
    // tcti_mssim->platform_handle = -1;
    ...
    // TODO:Rats-TLS can do with nonblock socket?
...
#ifdef RATS_TLS
    rats_tls_conf_t rats_tls_conf;
    memset(&rats_tls_conf, 0, sizeof(rats_tls_conf));
    rats_tls_conf.log_level = RATS_TLS_LOG_LEVEL_DEBUG;
    // Now, we can achieve "Baseline PASS" using nullattester-sgx_ecdsa(verifier) or nullattester-nullverifier for the Rats-TLS enhanced TSS
    strcpy(rats_tls_conf.attester_type, "nullattester");
    strcpy(rats_tls_conf.verifier_type, "sgx_ecdsa");
    strcpy(rats_tls_conf.tls_type, "openssl");
    strcpy(rats_tls_conf.crypto_type, "openssl");
    rats_tls_conf.cert_algo = RATS_TLS_CERT_ALGO_DEFAULT;
    // if (mutual)
    //rats_tls_conf.flags |= RATS_TLS_CONF_FLAGS_MUTUAL;

    rats_tls_handle tpm_handle;
    rats_tls_err_t ret = rats_tls_init(&rats_tls_conf, &tpm_handle);
    if (ret != RATS_TLS_ERR_NONE) {
        RTLS_ERR("Failed to initialize rats tls %#x\n", ret);
        goto fail_out;
    }
    ret = rats_tls_set_verification_callback(&tpm_handle, NULL);
    if (ret != RATS_TLS_ERR_NONE) {
        RTLS_ERR("Failed to set verification callback %#x\n", ret);
        goto fail_out;
    }
    ret = rats_tls_negotiate(tpm_handle, tcti_mssim->tpm_sock);
    if (ret != RATS_TLS_ERR_NONE) {
        RTLS_ERR("Failed to negotiate %#x\n", ret);
        rats_tls_cleanup(tpm_handle);
        goto fail_out;
    }

    tcti_mssim->tpm_handle = tpm_handle;
...
#ifdef RATS_TLS
    rats_tls_handle platform_handle;
    ret = rats_tls_init(&rats_tls_conf, &platform_handle);
    if (ret != RATS_TLS_ERR_NONE) {
        RTLS_ERR("Failed to initialize rats tls %#x\n", ret);
        goto fail_out;
    }
    ret = rats_tls_set_verification_callback(&platform_handle, NULL);
    if (ret != RATS_TLS_ERR_NONE) {
        RTLS_ERR("Failed to set verification callback %#x\n", ret);
        goto fail_out;
    }
    ret = rats_tls_negotiate(platform_handle, tcti_mssim->platform_sock);
    if (ret != RATS_TLS_ERR_NONE) {
        RTLS_ERR("Failed to negotiate %#x\n", ret);
        rats_tls_cleanup(platform_handle);
        goto fail_out;
    }
    tcti_mssim->platform_handle = platform_handle;
